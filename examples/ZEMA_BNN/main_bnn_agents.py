...
def main():
    ...
    bnn_agents = [agentNetwork.add_agent(agentType=BNN_Agent) for target in output_sizes]

    # evaluator_agent = agentNetwork.add_agent(agentType=EvaluatorAgent)
    evaluator_agents = [agentNetwork.add_agent(agentType=EvaluatorAgent) for target in output_sizes]
    ...
    for index,val in enumerate(output_sizes):
        bnn_agents[index].init_parameters(model=BNN_Model, output_size=val, selectY_col=index,architecture=architecture)
